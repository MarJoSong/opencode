...
function sessionErrorMessage(error: SessionError) {
  if (error?.name === "MessageAbortedError") return t("notification.session_aborted")
  const data = error?.data
  if (data && typeof data === "object" && "message" in data && data.message === "SSE read timed out") {
    return t("notification.model_stopped_responding")
  }
  return t("notification.session_error")
}
...
const tui: TuiPlugin = async (api) => {
  ...
  api.event.on("question.asked", (event) => {
    ...
    notify(api, event.properties.sessionID, t("notification.question_needs_input"), "question")
  })
  ...
  api.event.on("permission.asked", (event) => {
    ...
    notify(api, event.properties.sessionID, t("notification.permission_needs_input"), "permission")
  })
  ...
  api.event.on("session.status", (event) => {
    ...
    notify(api, sessionID, t("notification.session_done"), session?.parentID ? "subagent_done" : "done")
  })
